...
class UnsupportedStreamError(Exception):
    ...
    def __init__(self, stream_type: str, url: str):
        ...
        super().__init__(
            ...
            f"tapping. URL: {url}. This source requires browser-side capture."
        )
...
class URLResolver:
    """
    Resolves Eventyay stream_url and stream_type into a HLS manifest.
    """
    #Typers of streams supported by the resolver
    ...
    # stream_types with their own tap mechanisms
    #_FUTURE_TYPES = {StreamType.BBB, StreamType.JANUS}
    ...
    def resolve(self,stream_url:  str,stream_type: str,session_id:  str) -> ResolvedStream:
        ...
        # if stype in self._FUTURE_TYPES:
        #     return self._future_tap(stream_url, stype, session_id)

        if stype in self._DIRECT_HLS_TYPES:
            return self._direct_hls(stream_url, stype, session_id)

        #right now we are using YT-Steams
        if stype in self._YTDLP_TYPES:
            return self._resolve_via_ytdlp(stream_url, stype, session_id)

        # Should never reach here given the sets above are exhaustive
    ...
    def _resolve_via_ytdlp(self,page_url: str,stype: StreamType,session_id: str,) -> ResolvedStream:
        """
        YouTube and Vimeo provide human facing watch pages as stream_url,
        So we need to use yt-dlp to extract the real HLS manifest URL

        yt-dlp is called once per session, Subsequent calls within the same
        session return the cached URL immediately without re-running yt-dlp
        """
        # Cache hit 
    ...
    def _run_ytdlp(self, page_url: str) -> str:
        """
        Running yt-dlp as a subprocess to extract the best audio HLS manifest URL
        """
        # Use python -m yt_dlp to ensure we use the same environment
        cmd = [
    ...
    def _extract_hls_from_info(self, info: dict) -> Optional[str]:
        """
        Walks the yt-dlp dictionary looking for the best HLS audio format, 
        if not found then falls back to any m3u8 format
        """
    ...
    def _unsupported(
        ...
        Returns a ResolvedStream with tap_method=UNSUPPORTED and raises
        UnsupportedStreamError so the orchestrator can return an error to Eventyay.
        """
        logger.error(
